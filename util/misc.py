...
def openpose2numpy(pose):
    """
    Covnerts a raw JSON representation of OpenPose output to a numpy array

    :return: ndarray (nPoses, 25, 3[x,y,score])
    """

    for p in pose:
        assert len(p['pose_keypoints_2d']) == 75

    ret = [np.array(x['pose_keypoints_2d']) for x in pose]
    if len(ret) == 0:
        ret = np.zeros((0, 25, 3))
    else:
        ret = np.stack(ret, axis=0)
    return ret.reshape((-1, 25, 3))
