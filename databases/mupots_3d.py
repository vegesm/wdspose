...
MUPO_TS_PATH = os.environ['MUPOTS_FOLDER']
...
def load_2d_predictions(sequence, detector):
    ...
    if detector == 'openpose':
        return load(os.path.join('data', "openpose", sequence + '.pkl'), pkl_py2_comp=True)
    elif detector == 'hrnet':
        return load(os.path.join('data', "hrnet_matched", sequence + '.pkl'), pkl_py2_comp=True)
...
def load_jointwise_depth(sequence):
    return load(os.path.join('data', 'jointwise_depth', 'TS%02d.npy' % sequence))
...
def _scale_to_gt(pred_poses, gt_poses):
    ...
    assert_shape(pred_poses, (None, None, 3))
    assert_shape(gt_poses, (None, None, 3))
...
def eval_poses(matched_only, is_relative, pose3d_type, preds_2d_kpt, preds_3d_kpt):
    """
    ...
    :param preds_3d_kpt: seq->list(ndarray(nPoses,17,2)), in MuPo-TS joint order. 2D pose predictions.
